...
void Armor::Solvepnp()
{
    ...
    // tr = kalman_predict(tr);
    // bl = kalman_predict(bl);
    // br = kalman_predict(br);
    ...
    double dis = sqrt(tx * tx + ty * ty + tz * tz);
        double yaw = atan((double)tx / sqrt(pow(ty,2)+pow(tz,2)))*180/CV_PI;
    double pitch = atan((double) ty / sqrt(pow(tx,2)+pow(tz,2)))*180/CV_PI;
    this->pitch_angle = pitch;
    this->yaw_angle = yaw;
    vector<Point2f> cam;
    vector<Point3f> world = {Point3f(5, 0, 0), Point3f(0, 5, 0), Point3f(0, 0, 5)};
    ...
    putText(img, "0", point2d[0], 2, 1, Scalar(255, 0, 0));
    putText(img, "1", point2d[1], 2, 1, Scalar(255, 0, 0));
    putText(img, "2", point2d[2], 2, 1, Scalar(255, 0, 0));
    putText(img, "3", point2d[3], 2, 1, Scalar(255, 0, 0));
    this->distance = dis;
    this->x_angle = rx;
    this->y_angle = ry;
    this->z_angle = rz;
